...
@section('title', __('message.about_subtitle') . '- CV Fatih Jaya Film')
        ...
        <div class="flex flex-col lg:flex-row items-center px-6 gap-12">
            ...
            <div class="w-full lg:w-1/2 order-2 lg:order-1">
                <div class="container mx-auto relative z-10">
                    <div class="max-w-3xl">
                        <span class="text-brand-primary font-bold tracking-[0.2em] uppercase text-sm mb-4 block">{{ __('message.about_subtitle')}}</span>
                ...
                <div class="space-y-5 text-gray-300 leading-relaxed text-base prose prose-invert max-w-none">
                    {!! $about->description !!}
                    
                    <div class="flex flex-wrap gap-3 mt-6 not-prose">
                        ...
                        @empty
                            <div class="flex items-center gap-3">
                                <span class="text-brand-primary text-xl">✔</span>
                                <span class="font-bold">{{ __('message.service_car')}}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <span class="text-brand-primary text-xl">✔</span>
                                <span class="font-bold">{{ __('message.service_building')}}</span>
                            </div>
                        @endforelse
                    </div>

                    <div class="flex flex-wrap items-center gap-6 mt-10 not-prose">
    ...
    <div class="container mx-auto px-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
                ...
                <h3 class="text-2xl font-bold text-slate-900 mb-4">{{ __('message.mission_title')}}</h3>
                <div class="text-slate-600 rich-content prose prose-ul:list-disc prose-ul:pl-5 prose-li:marker:text-brand-primary">
                    {!! $about->mission !!}
                </div>
            </div>
        </div>
...
{{-- Section Values / Why Choose Us (CMS Repeater) --}}
<section class="py-12 bg-slate-900 text-white relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10">
        <div class="text-center mb-16">
            <span class="text-brand-primary font-bold uppercase tracking-widest text-sm">{{ __('message.values_subtitle')}}</span>
            <h2 class="text-3xl md:text-4xl font-bold mt-2">{{ __('message.values_title')}}</h2>
            <p class="text-slate-400 mt-4 max-w-2xl mx-auto">{{ __('message.values_main_desc')}}</p>
        </div>
...
{{-- Section Brand List (Tetap menggunakan logic Partner existing) --}}
<section class="py-12 bg-white border-b border-gray-100">
    <div class="container mx-auto px-6">
        <div class="container mx-auto px-4 mb-10 text-center">
            <span class="text-blue-600 font-semibold tracking-widest uppercase text-sm">{{ __("message.hero_subtitle")}}</span>
            <h2 class="text-4xl font-extrabold text-gray-900 mt-2">{{ __('message.partner_title')}}</h2>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach ($partners as $showPartner)
                 {{-- ... Code existing Partner ... --}}
                 <a href="{{ route('brand.detail', ['id' => $showPartner->id, 'slug' => Str::slug($showPartner->name)]) }}" 
                    ...
                    {{-- Content partner sama seperti sebelumnya --}}
                    ...
                    <div class="bg-gradient-to-r from-blue-500 to-blue-600 py-3 px-4">
                        <p class="text-white text-center text-xs font-bold uppercase tracking-widest">
                            {{ __('message.partner_brand_prefix')}} {{ $showPartner->name }}
                        </p>
                    </div>
                </a>
            @endforeach
...
{{-- Section CTA (Tetap Hardcode sesuai request, tapi link WA dinamis dari settings) --}}
<section class="py-12 bg-brand-primary relative overflow-hidden">
    <div class="absolute inset-0 bg-black/10"></div>
    <div class="container mx-auto px-6 relative z-10 text-center">
        <h2 class="text-3xl md:text-4xl font-bold text-white mb-6">{{ __('message.cta_title')}}</h2>
